...
LUXIA Cloud Vector Embedding API, cosine similarity, HDBSCAN 군집화, UMAP, 구역 분리 레이아웃, PCA 기반 쟁점 축 보기를 사용해 시민 의견 100건을 2D 의견 지도로 시각화하는 로컬 풀스택 프로토타입입니다.
...
7. 노이즈 의견 중 가까운 군집 중심이 있는 의견은 nearest centroid 방식으로 제한적으로 흡수합니다.
8. UMAP으로 정규화된 임베딩 벡터를 2차원 기본 좌표로 축소합니다.
9. 최종 군집 라벨을 기준으로 각 군집과 남은 미분류 의견을 별도 구역에 배치합니다.
10. PCA 2D 좌표를 사용해 쟁점 축 보기용 `issueX`, `issueY`와 축 양끝 라벨을 생성합니다.
11. React + Plotly.js 프론트엔드에서 구역 분리 보기와 쟁점 축 보기를 전환해 시각화합니다.
...
- UMAP 기반 2D 기본 좌표 생성
- HDBSCAN 기반 의미 군집화 및 가까운 노이즈 의견 흡수
- 구역 분리 레이아웃 기반 최종 지도 좌표 생성
- PCA 기반 쟁점 축 좌표와 축 양끝 라벨 생성
- 군집별 대표 의견 자동 선정
...
- 구역 분리 보기의 x축과 y축 자체에는 고정된 의미가 없습니다.
- 쟁점 축 보기는 PCA 2D 좌표를 사용하며, 각 축의 음수/양수 끝에 가까운 의견 category를 기준으로 축 라벨을 자동 생성합니다.
- `baseX`, `baseY`는 UMAP이 임베딩 벡터 간 거리 관계를 2차원으로 축소한 기본 좌표입니다.
- `islandX`, `islandY`는 군집과 미분류 의견을 별도 구역으로 나누기 위해 보정된 좌표입니다.
- `issueX`, `issueY`는 쟁점 축 보기용 PCA 좌표입니다.
- `x`, `y`는 현재 기본 보기 모드의 최종 표시 좌표입니다.
...
3. PCA 결과를 HDBSCAN으로 군집화하고 원래 HDBSCAN 라벨은 `hdbscanClusterId`로 보존합니다.
4. `-1` 노이즈 의견 중 가장 가까운 군집 centroid와의 유사도가 임계값 이상인 의견은 해당 군집으로 흡수합니다.
5. 각 최종 비노이즈 군집의 centroid와 가장 가까운 의견을 찾습니다.
6. 해당 의견을 `clusterRepresentative`로 사용합니다.
7. 대표 의견 또는 우세 category를 기준으로 `clusterName`을 생성합니다.
